...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (scanId) {
      api.get(`/results/${scanId}`).then(res => setResults(res.data)).catch(console.error);
    }
  }, [scanId]);
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-surface-container-high border-b border-t border-outline-variant/20 p-4 flex flex-col gap-4 shadow-xl z-50">
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="bg-primary text-on-primary font-headline font-bold text-xs uppercase tracking-widest px-4 py-2 rounded-sm active:scale-95 transition-all duration-200">
                Scan Website
              </button>
            </div>
          </div>
      ...
      <main className="pt-24 pb-12 px-4 md:px-8 max-w-7xl mx-auto space-y-8 w-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex flex-wrap gap-3 w-full md:w-auto">
            ...
            <Link to="/" className="flex-1 md:flex-none justify-center items-center gap-2 bg-primary text-on-primary px-4 py-2 font-label text-[10px] md:text-xs uppercase tracking-wider hover:brightness-110 transition-all flex">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          <div className="glass-card p-5 md:p-6 group hover:shadow-[0_0_30px_rgba(255,113,108,0.15)] transition-all">
            ...
            <div className="space-y-1">
              <div className="text-3xl md:text-4xl font-headline font-bold text-on-surface">{results ? results.broken_links : '--'}</div>
          ...
          <div className="glass-card p-5 md:p-6 group hover:shadow-[0_0_30px_rgba(159,142,255,0.15)] transition-all">
            ...
            <div className="space-y-1">
              <div className="text-3xl md:text-4xl font-headline font-bold text-on-surface">{results ? results.ui_issues : '--'}</div>
          ...
          <div className="glass-card p-5 md:p-6 group hover:shadow-[0_0_30px_rgba(161,250,255,0.15)] transition-all">
            ...
            <div className="space-y-1">
              <div className="text-3xl md:text-4xl font-headline font-bold text-on-surface">{results ? results.form_errors : '--'}</div>
          ...
          <div className="glass-card p-5 md:p-6 flex flex-col items-center justify-center relative overflow-hidden hidden sm:flex">
            <div className="absolute inset-0 flex flex-col items-center justify-center pt-2">
              <span className="text-xl md:text-2xl font-headline font-bold text-on-surface">{results ? results.health_score : '--'}%</span>
                  ...
                  {results && results.issues && results.issues.length > 0 ? (
                    results.issues.map((issue, idx) => (
                      <tr key={idx} className="hover:bg-surface-container-high/40 transition-colors group">
                        <td className="px-4 md:px-6 py-3 md:py-4 font-body text-xs md:text-sm text-on-surface-variant group-hover:text-primary transition-colors truncate max-w-[200px] block">{issue.page}</td>
                        <td className="px-4 md:px-6 py-3 md:py-4 font-body text-xs md:text-sm">{issue.description.substring(0,60)}...</td>
                        <td className="px-4 md:px-6 py-3 md:py-4">
                          <span className={`flex items-center gap-1.5 md:gap-2 text-[9px] md:text-[10px] font-label uppercase tracking-widest ${issue.severity === 'high' || issue.severity === 'critical' ? 'text-tertiary' : 'text-secondary'}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${issue.severity === 'high' || issue.severity === 'critical' ? 'bg-tertiary animate-pulse' : 'bg-secondary'}`}></span>
                            {issue.severity}
                          </span>
                        </td>
                        <td className="px-4 md:px-6 py-3 md:py-4">
                    ...
                    <tr>
                      <td colSpan="4" className="text-center py-6 text-on-surface-variant text-sm">
                        {results ? "No issues detected!" : "Loading scan data..."}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <div className="glass-card p-4 md:p-6 bg-primary/5 border-primary/20">
              <div className="flex items-center gap-2 md:gap-3 mb-4">
                ...
                <h3 className="font-headline text-xs md:text-sm font-bold uppercase tracking-widest">AI Architecture Insights</h3>
              </div>
              <ul className="space-y-3">
                {results && results.suggestions && results.suggestions.length > 0 ? (
                  results.suggestions.map((sug, i) => (
                    <li key={i} className="text-[10px] md:text-xs text-on-surface/90 leading-relaxed font-body flex items-start gap-2">
                      <span className="material-symbols-outlined text-[12px] text-primary mt-0.5">check_circle</span>
                      {sug}
                    </li>
                  ))
                ) : (
                  <li className="text-[10px] md:text-xs text-on-surface/50 italic">Running diagnostics to generate framework insights...</li>
                )}
              </ul>
              <button className="mt-6 w-full text-[10px] font-label uppercase tracking-[0.2em] py-2.5 bg-primary/10 text-primary border border-primary/30 hover:bg-primary/20 transition-colors cursor-pointer active:scale-95">Automate Patch Cycle</button>
            </div>
          </div>
        </div>
      </main>
